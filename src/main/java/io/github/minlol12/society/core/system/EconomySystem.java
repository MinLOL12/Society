...
public final class EconomySystem {
    ...
    private static void produce(SocietyEngine engine, Settlement s, Citizen citizen,
        ...
        switch (profession) {
            ...
            case HEALER:
                ...
                break;
            case BUILDER: {
                double livingSpace = s.housingCapacity();
                double wanted = population * 1.15 + s.tier().housingBase();
                if (livingSpace >= wanted * 1.4) break; // enough roofs for now
                double rate = 0.5 * skillFactor * s.tech().buildModifier();
                boolean timberFrame = s.stock(Good.WOOD) >= 0.8;
                if (timberFrame) {
                    s.addStock(Good.WOOD, -0.8);
                    s.addStock(Good.STONE, -Math.min(s.stock(Good.STONE), 0.5));
                } else {
                    rate *= 0.35; // rough lean-tos of whatever lies around
                }
                s.addHousing(rate);
                grantWorkXp(engine, citizen, primary, 0.9);
                break;
    ...
    static void computeNeeds(SocietyEngine engine, Settlement s, int population, double security) {
        ...
        setDemand(s, SimProfession.LUMBERJACK,
                deficit(s, Good.WOOD) * 1.3);
        setDemand(s, SimProfession.MINER,
                Math.max(deficit(s, Good.IRON), deficit(s, Good.STONE)) * 1.2);
        double housingShort = (population + 2) - s.housingCapacity();
        setDemand(s, SimProfession.BUILDER,
                housingShort > 0 ? 1.7 : 0.15);
    ...
    private static void maybeRetrain(SocietyEngine engine, Settlement s, Citizen c, int population, Random random) {
        ...
        if (!foodCrisis && currentDemand > 0.15) return;
        if (!foodCrisis && s.professionDemand(SimProfession.BUILDER) < 1.2
                && s.professionDemand(SimProfession.FARMER) < 1.2) return;
        if (random.nextDouble() >= 0.08) return;
        // Retrain into the sharpest need that fits.
        SimProfession best = null;
        double bestDemand = 1.0;
        for (SimProfession p : new SimProfession[]{SimProfession.FARMER, SimProfession.BUILDER, SimProfession.MINER, SimProfession.LUMBERJACK}) {
            double demand = s.professionDemand(p);
            if (demand > bestDemand && s.professionCount(p) < professionCap(p, population)) {
                best = p;
                bestDemand = demand;
            }
        }
